...
img_dir='/home/pej/platform_yolo/NEW_CLASSIFIED/delete/regular'
label_dir='/home/pej/platform_yolo/NEW_CLASSIFIED/delete/regular_label'

print("디렉토리 생성")
newdir = img_dir + "_mixed/"
os.makedirs(newdir)
...
for i in range(len(images)):
    ...
    if p < 1.0:
        ...
        # padding to 1024 * 768
        # constant
        padded = cv2.copyMakeBorder(resize, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        ...
        for j in range(len(labels[i])):
            ...
            x = (float(labels_i[j][1]) * w_new + mag_x_start) / float(w)
            y = (float(labels_i[j][2]) * h_new + mag_y_start) / float(h)
        ...
        cv2.imwrite(result_folder + str(p) + "_" + image_names[i], padded)
